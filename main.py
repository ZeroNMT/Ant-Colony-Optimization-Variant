...
pheromne = .1*np.ones((m, n))  #intializing pheromne present at the paths to the cities
...
for ite in range(iteration):
    ...
    for i in range(m):
        ...
        for j in range(n-2): # expect initial point and last point
        ...
        last_point = list(set([i for i in range(1, n+1)])-set(rute[i, :-2]))[0] # finding the last untraversed city to route
        rute[i, -2] = last_point  # adding untraversed city to route
    ...
    for i in range(m):
        s = 0
        for j in range(n-1):
            s = s + dist_matrix[int(rute_opt[i, j])-1, int(rute_opt[i, j+1])-1] # calcualting total tour distance
